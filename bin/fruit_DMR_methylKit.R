...
ref=args[3]
ref2compare<-list.files(path= path2acc, pattern = paste0(chr2process, ".filtered.bed"), full.names = FALSE)
ref2compare<-grep(paste0(args[3], "_Biseq_C"), ref2compare, value =TRUE )
...
data.table::fwrite(CG_DMR.toSave, file=paste0(path2results,comparison,"_", chr2process,"_",accName,"_CG_DMR.methylation.bed"),
...
data.table::fwrite(CHG_DMR.toSave, file=paste0(path2results,comparison,"_", chr2process,"_",accName,"_CHG_DMR.methylation.bed"),
                   quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
...
data.table::fwrite(CHH_DMR.toSave, file=paste0(path2results,comparison,"_", chr2process,"_",accName,"_CHH_DMR.methylation.bed"),
                   quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
