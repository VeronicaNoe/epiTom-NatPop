...
path2acc<-/mnt/disk2/vibanez/02_methylkit/ab_preprocessing/ac_CG-DMR/bb_output/"
path2results<-"/mnt/disk2/vibanez/02_methylkit/ac_mergeDMRs/bb_output/"
...
allSamples<-grep(".bed". allSamples. value=TRUE)
...
CG<-cbind(CG.df, ctxt)
colnames(CG)<-toName
data.table::fwrite(CG, file=paste0(path2results, chr2process,"_merged_CG-DMR.bed"), quote=F,row.names=F,col.names = T,sep="\t")
...
#colnames(CHG)<-toName
#data.table::fwrite(CHG, file=paste0(path2results, chr2process,"_merged_CG-DMR.bed"), quote=F,row.names=F,col.names = T,sep="\t")
