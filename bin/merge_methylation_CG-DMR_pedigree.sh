...
intersectBed -a - -b ${LOCI}/"CG-DMR-loci_collapsed.bed" > ${OUT_DIR}/"CG-DMR_"${GEN}".merged.methylation.bed"

ls "${SAMPLE_DIR}"/*"_CG-DMR_"${GEN}.methylation.bed |\
    xargs -n 1 basename |\
    cut -d'_' -f1 > "${OUT_DIR}/CG-DMR_"${GEN}".colNames.tsv"
