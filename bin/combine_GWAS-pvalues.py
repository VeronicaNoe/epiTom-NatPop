...
def combine_pvalues_for_chunk(pvalue_array):
    combined_pvalues = np.apply_along_axis(lambda row: combine_pvalues(row, method='fisher')[1], 1, pvalue_array)
...
def main():
    ...
    with open(os.path.join(output_dir, f'combined_pvalues_{dmr_type}.csv'), 'w') as outfile:
        outfile.write("key\tcombined_pvalue\tfile_count\n")
        for chunk in range(num_chunks):
            ...
            combined_pvalues = combine_pvalues_for_chunk(pvalue_array)
            for i in range(chunk_size):
                if keys[i] is not None:
                    outfile.write(f"{keys[i]}\t{combined_pvalues[i]:.6e}\t{file_counts[i]}\n")
