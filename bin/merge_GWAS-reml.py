...
import sys
args = sys.argv[1:]
# The first element in sys.argv is the script filename, so we skip it
...
samples = [f for f in os.listdir(WD) if fnmatch.fnmatch(f, f"{toProcess}*.reml")]
...
with open(os.path.join(OUTDIR, f"{args[0]}_{args[1]}_{args[2]}.reml"), 'w') as output_file:
