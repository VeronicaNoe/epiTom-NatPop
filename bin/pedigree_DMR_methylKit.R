...
#CG_DMR.toSave<-subset(CG_DMR, (meth.diff <= -50 | meth.diff >= 50))
...
data.table::fwrite(CG_DMR, file=paste0(path2results, chr2save,"_" ,controlAcc,"_",sampleAcc,"_CG_DMR.wo-filtering"),
                    quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
...
#CHG_DMR.toSave<-subset(CHG_DMR, (meth.diff <= -50 | meth.diff >= 50))
data.table::fwrite(CHG_DMR.toSave, file=paste0(path2results, chr2save,"_",controlAcc,"_",sampleAcc,"_CHG_DMR.methylation.bed"),
                   quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
data.table::fwrite(CHG_DMR, file=paste0(path2results, chr2save,"_",controlAcc,"_",sampleAcc,"_CHG_DMR.wo-filtering"),
                    quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
cat(paste0("---------- CHH", "\n"))
...
#CHH_DMR.toSave<-subset(CHH_DMR,(meth.diff <= -10 | meth.diff >= 10))
data.table::fwrite(CHH_DMR.toSave, file=paste0(path2results, chr2save,"_",controlAcc,"_",sampleAcc,"_CHH_DMR.methylation.bed"),
                   quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)

data.table::fwrite(CHH_DMR, file=paste0(path2results, chr2save,"_",controlAcc,"_",sampleAcc,"_CHH_DMR.wo-filtering"),
                    quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
