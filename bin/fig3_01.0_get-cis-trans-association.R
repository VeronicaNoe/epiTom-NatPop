...
data.table::fwrite(summ, file= paste0(outDir,'01.2_cis-sig-associations_summary.tsv'), quote=F,
                   row.names=F,col.names = T, sep="\t")
## make a venn
summ %>%
  group_by(DMR, type) %>%
...
ggsave(paste0(outPlot, "00.0_DMRs-association_cis-trans_piePlot.pdf"), width = 12, height = 8)
