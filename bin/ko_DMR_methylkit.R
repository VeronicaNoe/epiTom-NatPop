...
#set paths and chr
path2acc<-"/mnt/disk2/vibanez/01_bismark_analysis/04_processingFiles/ac_filter/tmp2"
path2results<-"/mnt/disk2/vibanez/02_methylkit/aa_getDMR/tmp_output/"
chr2process=args[2]
#chr2process="ch01"
#
acc<-list.files(path=path2acc, pattern =paste0(chr2process,".filtered.bed") , full.names = FALSE)
acc<-grep("WT-25_R1_pe", acc, value=TRUE, invert=TRUE)

ref2compare<-list.files(path= path2acc, pattern = "WT-25_R1_pe", full.names = FALSE)
...
allSamples<-gsub(paste0(path2acc,"/"), "", allSamples)
allSamples<-gsub(paste0("_CG_",chr2process,".filtered.bed"),'', allSamples)
sampleNames<-as.list(c(allSamples)) #Definir el nombre de las muestras para methylkit
ctrlContrast<-0
sampleContrast<-c(1,1,1)
...
data.table::fwrite(CG_DMR.toSave, file=paste0(path2results, chr2process,"_",accName,"_CG_DMR.methylation.bed"),
...
data.table::fwrite(CHG_DMR.toSave, file=paste0(path2results, chr2process,"_",accName,"_CHG_DMR.methylation.bed"),
                   quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
...
data.table::fwrite(CHH_DMR.toSave, file=paste0(path2results, chr2process,"_",accName,"_CHH_DMR.methylation.bed"),
                   quote=F,row.names=F,col.names = F,sep="\t", nThread=20, buffMB=1024)
